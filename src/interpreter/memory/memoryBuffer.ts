...
export default class MemoryBuffer {
    ...
    constructor(node_size: number,
        ...
        this.mem_size = heap_size
        this.mem_size_words = heap_size * word_size
        this.mem_make(this.mem_size_words)
    }
    ...
    public mem_get = (address: number) =>
        this.memoryView.getFloat64(address * this.word_size)
    ...
    public mem_get_tag = (address: number) =>
        this.memoryView.getInt8(address * this.word_size)

    public mem_get_size = (address: number) =>
        this.memoryView.getUint16(address * this.word_size +
            this.size_offset)
    ...
    public mem_get_number_of_children = (address: number) =>
        this.mem_get_tag(address) === TAGS.int_tag
            ? 0
            : this.mem_get_size(address) - 1

    // access byte in heap, using address and offset
    public mem_set_byte_at_offset =
        (address: number, offset: number, value: number) =>
            this.memoryView.setUint8(address * this.word_size + offset, value)

    public mem_get_byte_at_offset =
