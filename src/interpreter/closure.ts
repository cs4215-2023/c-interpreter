...
import { Function, Identifier } from '../parser/types'
...
const closureToJS = (value: Closure, context: Context, klass: string) => {
  function DummyClass(this: Closure) {
    ...
    const gen = apply(
      ...
      value,
      args,
      callExpression(identifier(klass, undefined), args),
      this
    )
    let it = gen.next()
    while (!it.done) {
...
export default class Closure extends Callable {
  ...
  public originalNode: Function

  constructor(public node: Function, public environment: Environment, context: Context) {
    ...
    if (this.node.type === 'FunctionDeclaration' && this.node.id !== null) {
      ...
      this.functionName =
        ...
        this.node.params.map((o: Identifier) => o.name).join(', ') +
        (this.node.params.length === 1 ? '' : ')') +
        ' => ...'
    }
